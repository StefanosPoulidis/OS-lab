...
//create an array to hold all my children's PIDs so as to send SIGCONT 
	...
	if (root->nr_children == 0){
		...
		printf("Process %s started creating children\n",root->name);
                for (i=0;i<root->nr_children;i++){ //max na treksei 2 fores afou exoume 2  paidia  h kanena
                       pid=fork();  
	              	if (pid<0){
				...
				close(filedesnew[0]); //child is write-only
                                let_it_fork(root->children+i,filedesnew);          //child, recursive call gia ka8e pointer se child
				printf("Process %s : Exiting\n",root->name); 
                               exit(0);
                        }
	...
	//////	printf("PID= %ld, name = %s is waiting for SIGCONT\n",(long)getpid(),root->name);
		...
		//edw prepei na steilei SIGCONT diadoxika sta paidia ths kai na perimenei na teleiwsoun o,ti exoun na kanoun
